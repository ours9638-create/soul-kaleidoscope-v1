...
profile.outputs.report = Report.generate(profile, snglData);

const full = ReportModel.build(profile, {
  mode: "full",
  notes: { overview: "整體觀察", focus: "當期重點", recommendations: "補充建議" }
});
check("報告模型版本", ReportModel.VERSION, "1.0.0");
...
check("SNGL 四段", full.interpretations.length, 4);
...
const custom = ReportModel.build(profile, { mode: "quick", visibility: { stages: true, notes: false } });
...
check("快速版文字不含五階段標題", quickText.includes("【五階段數字】"), false);

const fullText = ReportModel.plainText(full);
check("完整版文字包含五階段", fullText.includes("【五階段數字】"), true);
...
check("老師版包含版本", teacherText.includes("Report View 1.0.0"), true);
...
console.log(`Report view model ${ReportModel.VERSION}; modes ${Object.keys(ReportModel.MODES).join(", ")}.`);
